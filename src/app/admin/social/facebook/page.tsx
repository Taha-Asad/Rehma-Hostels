// app/admin/social/facebook/page.tsx
"use client";
...
import { fetchFacebookAnalytics } from "@/actions/social.action";
...
export default function FacebookPage() {
  ...
  const loadData = async (range: DateRange) => {
    startTransition(async () => {
      const result = await fetchFacebookAnalytics(range);
  ...
  return (
    <Box>
      ...
      <Grid container spacing={3} mb={4}>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatsCard stat={stats.likes} icon={<ThumbUp />} color="#f44336" />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatsCard
            stat={stats.comments}
      ...
      {activeTab === 0 && (
        <PostsGrid posts={data.topPosts} title="Top Performing Posts" />
      )}
      {activeTab === 1 && (
        <PostsGrid posts={data.recentPosts} title="Recent Posts" />
      )}
      {activeTab === 2 && (
